...
class Pagination(object):
    ...
    def page_html(self):
        ...
        #首页
        self.params['page'] = 1
        first_page = '<li><a href="%s?%s"aria-label="Previous">首页</a></li>' % (self.base_url,self.params.urlencode(),)
        page_html_list.append(first_page)
        # 上一页
        if self.params!=1:
            self.params['page']=self.current_page-1
            previous_page = '<li><a href="%s?%s" aria-label="Previous"><span class="glyphicon glyphicon-menu-left"></span></a></li>'% (self.base_url,self.params.urlencode(),)#拼接url
        ...
        for i in range(pager_start, pager_end + 1):
            ...
            if i == self.current_page:
                #点击选择的页
                temp = '<li class="active"><a  href="%s?%s">%s<span class="sr-only">(current)</span></a></li>' % (self.base_url,self.params.urlencode(), i,)
            else:
                #未点击选择的页
        ...
        if self.params['page']!=self.max_page_num:
            self.params['page'] = self.current_page +1
            next_page = '<li><a href="%s?%s" aria-label="Previous"><span class="glyphicon glyphicon-menu-right"></span></a></li>' % (self.base_url, self.params.urlencode(),)
